...
class SmbcPromptSpec extends SpecificationLike {
  "SmbcPromptConversion" >> {
    "#apply" >> {
      "it generates OFX statement" in {
        val config = ConfigFactory.load().getConfig("net.shiroka.tools.ofx.conversions.smbc-prompt")
